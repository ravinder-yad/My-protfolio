    ...
    <div className="w-full lg:w-2/3 space-y-16 pb-32">
      ...
      <section id="projects" className="scroll-mt-32">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-3xl font-extrabold text-textMain">Featured Projects</h2>
        </div>
        ...
        <div className="space-y-6">
          {projectsData.map((project, index) => (
            <motion.div
              ...
              className="bg-white p-8 rounded-3xl border border-gray-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_10px_30px_rgb(37,99,235,0.08)] hover:border-blue-100 transition-all group"
            >
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 gap-2">
                <h3 className="text-2xl font-bold text-textMain group-hover:text-primary transition-colors">
                  {project.title}
                </h3>
                <span className="px-3 py-1 bg-gray-50 text-textGray text-xs font-bold uppercase tracking-wider rounded-lg border border-gray-200">
                  {project.duration}
                </span>
              </div>
              ...
              <div className="pt-6 border-t border-gray-50">
                <p className="text-xs font-bold text-textMain uppercase tracking-widest mb-3">Key Features</p>
                <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  {project.features.map((feature, i) => (
                    <li key={i} className="flex items-center text-sm text-textGray">
      ...
      <section id="skills" className="scroll-mt-32">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-3xl font-extrabold text-textMain">Technical Skills</h2>
        </div>
      ...
      <section id="education" className="scroll-mt-32">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2 className="text-3xl font-extrabold text-textMain">Education & Certifications</h2>
        </div>
        ...
        <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-blue-200 before:to-transparent">
          ...
          <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
            ...
            <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-6 rounded-3xl bg-white border border-gray-100 shadow-sm">
              <div className="font-bold text-primary mb-1">{portfolioData.personalInfo.duration}</div>
              <h4 className="text-xl font-bold text-textMain mb-1">{portfolioData.personalInfo.education}</h4>
          ...
          {portfolioData.certifications.map((cert, index) => (
            <div key={index} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
              ...
              <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-6 rounded-3xl bg-white border border-gray-100 shadow-sm">
                ...
                <h4 className="text-lg font-bold text-textMain mb-1">{cert.title}</h4>
                <p className="text-sm text-textGray">{cert.organization}</p>
              </div>
            </div>
          ))}
...
const SkillCard = ({ title, skills, color }) => {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm p-6 rounded-2xl border border-gray-100 shadow-sm">
      <h3 className="text-xl font-bold text-textMain mb-4">{title}</h3>
