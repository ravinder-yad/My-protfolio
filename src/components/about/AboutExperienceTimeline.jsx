      ...
      <div className="container mx-auto px-6 max-w-4xl">
        <div className="text-center mb-16">
          <motion.h2 
            ...
            viewport={{ once: true }}
            className="text-4xl md:text-5xl font-extrabold text-textMain mb-4"
          >
        ...
        <div className="relative mt-16 pl-4 md:pl-0">
          ...
          <div className="absolute left-6 md:left-[50%] top-0 bottom-0 w-1 bg-gray-100 rounded-full transform md:-translate-x-1/2 overflow-hidden">
            <motion.div 
    ...
    >
      ...
      <div className="absolute left-6 md:left-[50%] w-6 h-6 transform -translate-x-1/2 mt-6 md:mt-0 z-20 flex items-center justify-center">
        <motion.div 
          ...
          className="w-6 h-6 rounded-full bg-white border-4 border-primary shadow-[0_0_15px_rgba(37,99,235,0.6)]"
        ></motion.div>
      </div>

      {/* Content Box */}
      <div className={`ml-14 md:ml-0 w-full md:w-1/2 ${alignLeft ? 'md:pl-16' : 'md:pr-16 md:text-right'}`}>
        <div className="group bg-white p-8 rounded-3xl shadow-sm border border-gray-100 hover:shadow-lg transition-all duration-300 relative overflow-hidden">
          ...
          <h3 className="text-2xl font-bold text-textMain mb-3 group-hover:text-primary transition-colors">{data.title}</h3>
          <p className="text-textGray leading-relaxed">{data.description}</p>
        </div>
      </div>
    </motion.div>
  );
