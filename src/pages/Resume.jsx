...
const Resume = () => {
  ...
  return (
    <div className="min-h-screen bg-white pt-28 pb-20 font-sans text-gray-800">
      ...
      >
        ...
        {/* SUMMARY SECTION */}
        <div className="mb-16">
          <h3 className="text-2xl font-bold text-gray-900 uppercase tracking-widest mb-6 flex items-center gap-4">
            Professional Summary
            <span className="h-px flex-1 bg-gray-200"></span>
          </h3>
          <p className="text-lg leading-relaxed text-gray-600 max-w-5xl">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {skills.frontend.map((skill, i) => (
                  <span key={i} className="px-3 py-1.5 bg-gray-50 text-gray-700 rounded-lg text-sm border border-gray-100">{skill}</span>
                ))}
              </div>
            </div>
            <div>
        ...
        {/* PROJECTS SECTION */}
        <div className="mb-16">
          <h3 className="text-2xl font-bold text-gray-900 uppercase tracking-widest mb-8 flex items-center gap-4">
            Projects & Experience
            <span className="h-px flex-1 bg-gray-200"></span>
          </h3>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {projectsData.map((project, index) => (
              <div key={index} className="p-8 rounded-2xl border border-gray-100 bg-white shadow-sm hover:shadow-md transition-shadow group flex flex-col h-full">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {project.techStack.map((tech, i) => (
                    <span key={i} className="text-xs font-bold text-gray-600 bg-gray-100 px-2 py-1 rounded">
                      {tech}
                    </span>
                  ))}
                </div>
                ...
                <div className="pt-4 border-t border-gray-100 mt-auto print:hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div className="space-y-8">
              <div className="relative pl-6 border-l-4 border-blue-500">
                <div className="absolute w-4 h-4 bg-white border-4 border-blue-500 rounded-full -left-[10px] top-1"></div>
                <h4 className="text-xl font-bold text-gray-900 mb-1">{personalInfo.education}</h4>
              ...
              <div className="relative pl-6 border-l-4 border-indigo-500">
                <div className="absolute w-4 h-4 bg-white border-4 border-indigo-500 rounded-full -left-[10px] top-1"></div>
                <h4 className="text-xl font-bold text-gray-900 mb-1">{personalInfo.training.split(' - ')[0]}</h4>
                <div className="text-lg text-indigo-600 font-semibold mb-2">{personalInfo.trainingDuration}</div>
                <p className="text-gray-600">Tips-G Alwar</p>
              </div>
            </div>

            {/* Certifications Right Column */}
            <div>
              <h4 className="text-lg font-bold text-gray-900 mb-6">Additional Certifications</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {certifications.filter(c => c.organization !== "Tips-G Alwar").map((cert, i) => (
                  <div key={i} className="p-4 rounded-xl border border-gray-100 bg-gray-50">
                    ...
                    <div className="text-xs text-gray-500 mt-1">{cert.duration}</div>
                  </div>
                ))}
              </div>
            </div>
